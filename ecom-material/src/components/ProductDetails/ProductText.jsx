...
const ProductText = ({ product, onAddToCart }) => {
  const [selectedVariant, setSelectedVariant] = useState(product.variant_groups[0].options[0]);

  const variantGroup = (variantgrp) => {
    return variantgrp.options.map((variant) => (
      <IconButton color="inherit" key={variant.name} onClick={() => setSelectedVariant(variant)}>
        {variant.id === selectedVariant.id ? (
          <CheckCircle sx={{ color: `#${variant.name}`, border: `1.5px solid ${variant.id === selectedVariant.id ? "#000" : "#b3b3b3"}`, borderRadius: "50%" }} />
        ) : (
          <Circle sx={{ color: `#${variant.name}`, border: `1.5px solid ${variant.id === selectedVariant.id ? "#000" : "#b3b3b3"}`, borderRadius: "50%" }} />
        )}
      </IconButton>
    ));
  };
  ...
  return (
    <>
      <Paper style={{ padding: "1rem" }} variant="outlined" square>
        ...
        <Button variant="contained" disableElevation sx={{ mt: 3 }} onClick={() => onAddToCart(product.id, 1, product.variant_groups[0].id, selectedVariant.id)}>
          Add To Cart
        </Button>
      </Paper>
    </>
